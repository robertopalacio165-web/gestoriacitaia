...
export default async function handler(
  ...
  try {
    ...
    // ============================================================
    // BREVO SMTP — MISMO SISTEMA QUE TRABAJO EN MALTA
    // ============================================================
    ...
    if (!smtpUser || !smtpPass) {
      console.error("❌ Faltan SMTP_USER o SMTP_PASS");
    ...
    const confirmationPdfUrl = String(
      pdfUrl ||
        process.env.STUDY_MALTA_CONFIRMATION_PDF_URL ||
        DEFAULT_PDF_URL
    ).trim();
    ...
    if (!pdfResponse.ok) {
      throw new Error(
        `No se pudo descargar el PDF. HTTP ${pdfResponse.status}`
      );
    }
    ...
    if (pdfBuffer.length < 1000) {
      throw new Error(
        "El PDF descargado no es válido."
      );
    }

    const pdfFileName =
      `GestoriaCitaIA-Estudiar-Malta-2027-${cleanFileName(
        fullName
      )}.pdf`;
...
style="width:100%;max-width:680px;background:#ffffff;border-radius:18px;overflow:hidden;">
...
style="border:1px solid #dfe6ef;border-radius:14px;margin:20px 0 25px;">
...
`;
    ...
    console.log("🔌 Verificando SMTP Brevo...");

    await transporter.verify();

    console.log("✅ SMTP BREVO OK");
    console.log("📧 Enviando Estudios Malta...");
