...
export default async function handler(
  ...
  if (event.type === "checkout.session.completed") {
    ...
    // ============================================
    // 🚫 BLOQUEO ESTUDIOS MALTA 2027
    // Este webhook es SOLO para TRABAJO MALTA
    // ============================================

    if (metadata.service === "study_malta_2027") {
      console.log("=========================================");
      console.log("⏭️ PAGO DE ESTUDIOS MALTA 2027 DETECTADO");
      console.log("Stripe Session:", session.id);
      console.log("Service:", metadata.service);
      console.log("⛔ NO malta_applications");
      console.log("⛔ NO CV");
      console.log("⛔ NO Cover Letter");
      console.log("⛔ NO email Trabajo Malta");
      console.log("⛔ NO worker_queue");
      console.log("⛔ NO Make new_job");
      console.log("=========================================");
      ...
      return res.status(200).json({
        ...
        message:
          "Study Malta payment ignored by Malta Jobs webhook",
      });
    }
    ...
    const ingles_nivel =
      metadata.ingles_nivel || "";
    const frances_nivel =
      metadata.frances_nivel || "";
    const italiano_nivel =
      metadata.italiano_nivel || "";
    const espanol_nivel =
      metadata.espanol_nivel || "";
    const arabe_nivel =
      metadata.arabe_nivel || "";
    const aleman_nivel =
      metadata.aleman_nivel || "";
    ...
    const trabajo_busca =
      metadata.trabajo_busca || "";

    const experiencia_previa =
      metadata.experiencia_previa || "";

    const anos_experiencia =
      metadata.anos_experiencia || "";
    ...
    const carnetConducir =
      metadata.carnetConducir || "";
    ...
    const {
      ...
      .select(
        "id, worker_status, photo_url, pdf_url"
      )
    ...
    if (existing) {
      ...
      console.log(
        `🔄 Actualizando aplicación existente: ${applicationId}`
      );
      ...
      const updateData: any = {
        ...
        // Usar nuevos valores si vienen
        // sino mantener los existentes
        photo_url:
      ...
      if (session.payment_intent) {
        updateData.stripe_payment_intent =
      ...
      console.log(
        `✅ Aplicación ${applicationId} actualizada correctamente`
      );
    } else {
      // ============================================
      ...
      console.log(
        `✅ Registro creado en Supabase: ${applicationId}`
      );
    ...
    if (isNew) {
      console.log(
        `📄 Generando documentos para ${applicationId}`
      );
      ...
      console.log(
        `📧 Enviando email de bienvenida para ${applicationId}`
      );
...
📤 Daily applications:
immediately after your documents are ready.
...
`,
      ...
      // ============================================
      // 11. AÑADIR A LA COLA DE TRABAJO
      // SOLO SI ES NUEVO
      // ============================================
      ...
      try {
        ...
        if (queueError) {
          console.error(
            "❌ Error adding to worker queue:",
            queueError
          );
        } else {
          console.log(
            `✅ Añadido a la cola de trabajo: ${applicationId}`
          );
        }
      } catch (queueErr) {
        console.error(
          "❌ Worker queue exception:",
          queueErr
        );
      }
    } else {
      console.log(
        `⏳ Aplicación ${applicationId} ya existe, no se procesa`
      );
    }
    ...
    try {
      ...
      console.log(
        `✅ Notificado a Make: ${applicationId}`
      );
    } catch (err) {
      console.error(
        "❌ Error notificando a Make:",
        err
      );
    }
