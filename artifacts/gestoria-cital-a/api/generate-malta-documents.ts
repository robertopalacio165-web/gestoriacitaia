...
// ============================================
// ✅ GENERAR CARTA CON IA - CORREGIDA (SIN ERRORES DE SINTAXIS)
// ============================================

async function generatePremiumCoverLetter(data: any, company: Company, jobTitle: string): Promise<LetterContent> {
  // ✅ PROMPT EN UNA SOLA LÍNEA - SIN ERRORES DE SINTAXIS
  const prompt = `You are a professional cover letter writer for the European job market. Rules: Maximum 300 words, British English, sound like a human. Mention the company name, Malta, relocation, teamwork and motivation. Never sound generated by AI. Never repeat sentences. End with a professional call to action. Candidate: Name: ${data.full_name || "Candidate"}, Position: ${jobTitle}, Company: ${company.name || "Hilton Malta"} (${company.city || "Malta"}), Experience: ${data.anos_experiencia || "3-5 years"}, Nationality: ${data.nationality || "Moroccan"}, Current City: ${data.current_city || "Casablanca"}, Education: ${data.education_level || "Secondary Education"}, Languages: ${data.idiomas || "English, Arabic, French"}. Generate a professional cover letter body as JSON with these fields: introduction, body1, body2, body3, closing. Each paragraph 2-4 sentences. Professional British English tone. Be specific about the company and position. Sound human, not AI-generated. Make it unique for this candidate.`;
...
// ============================================
// GENERAR HTML DE LA CARTA - MEJORADA
// ============================================
