...
// ============================================
// FUNCIONES DE NORMALIZACIÓN - LIMPIAS
// ============================================
...
async function copyFolder(
  ...
  for (const file of files) {
    ...
    await supabase.storage
      .from(BUCKET_NAME)
      .upload(`${destinationFolder}/${file.name}`, bytes, {
        upsert: true,
      });
  }
}
...
export default async function handler(
  ...
  try {
    ...
    // --- NUEVO: Copiar archivos existentes desde malta-temp ---
    console.log(`📂 Copying files from malta-temp to ${folder}...`);
