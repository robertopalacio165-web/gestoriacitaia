...
// ============================================
// GENERAR CARTA CON IA - VERSIÓN SIMPLE (SIN ERRORES)
// ============================================

async function generatePremiumCoverLetter(data: any, company: Company, jobTitle: string): Promise<LetterContent> {
  const prompt = `You are a professional cover letter writer. Candidate: ${data.full_name || "Candidate"}, Position: ${jobTitle}, Company: ${company.name || "Hilton Malta"}, Experience: ${data.anos_experiencia || "3-5 years"}, Nationality: ${data.nationality || "Moroccan"}, City: ${data.current_city || "Casablanca"}, Education: ${data.education_level || "Secondary Education"}, Languages: ${data.idiomas || "English, Arabic, French"}. Generate JSON with introduction, body1, body2, body3, closing. 2-4 sentences each. British English. Mention Malta and relocation. Professional tone.`;
