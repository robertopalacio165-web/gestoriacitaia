...
src="${LOGO_URL}"
alt="GestoriaCitaIA"
style="width:300px;max-width:90%;height:auto;display:block;margin:0 auto 18px auto;"
/>
...
<h2 style="font-size:22px;margin:25px 0 10px;">
...
<strong dir="ltr" style="font-size:17px;">
${safeWhatsapp}
</strong>
...
<h2 style="margin:0 0 12px;color:#0b57d0;font-size:19px;">
...
<strong>
مسطرة التسجيل فـ مركز ديال اللغة الإنجليزية فمالطا 🇲🇹
</strong>.
...
<h3 style="font-size:19px;margin:28px 0 15px;">
المراحل الجاية
</h3>
...
غادي نشرحو ليك فالمكالمة الوثائق والخطوات اللي خاصك تكمل من بعد.
...
تاريخ الازدياد: ${safeDob || "—"}

<br>

الجنسية: ${safeNationality || "—"}
...
شكراً بزاف على الثقة ديالك فـ <strong>GestoriaCitaIA</strong>.
...
<div class="value" dir="ltr">${w}</div>
</td>

<td>
<div class="label">البريد الإلكتروني</div>
<div class="value" dir="ltr">${e}</div>
</td>
...
فمدة أقصاها
<strong style="color:#07853f;">24 ساعة ديال الخدمة</strong>،
غادي نتاصلو بيك فالرقم اللي عطيتينا باش نراجعو معاك المعلومات ونبداو
    ...
    await page.evaluate(async () => {
      ...
      try {
        ...
        await document.fonts.load(
          '700 24px "Noto Naskh Arabic"'
        );

        await document.fonts.load(
          '400 16px "Noto Naskh Arabic"'
        );
...
export async function sendWelcomeEmail({
  ...
  if (!email) {
    ...
    throw new Error(
      "No se ha recibido el email del cliente."
    );
  ...
  const info =
    await transporter.sendMail({
      ...
      from:
        `"${process.env.FROM_NAME || "GestoriaCitaIA"}" <${process.env.FROM_EMAIL || "jobs@gestoriacitaia.com"}>`,

      to:
        normalizedEmail,

      subject:
        `🇲🇹 تأكيد طلب الدراسة فمالطا 2027 · ${fullName}`,
  ...
  console.log(
    "✅ EMAIL + PDF ESTUDIOS MALTA ENVIADOS POR BREVO"
  );
