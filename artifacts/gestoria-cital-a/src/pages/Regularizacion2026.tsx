...
export default function Regularizacion2026() {
  ...
  const [asiloStatus, setAsiloStatus] = useState<{ tieneSolicitud: boolean; tieneDenegacion: boolean; mensaje: string }>({ tieneSolicitud: false, tieneDenegacion: false, mensaje: "" });
  const [expulsionStatus, setExpulsionStatus] = useState<{ tieneActiva: boolean; fechaCaducidad: Date | null; mensaje: string }>({ tieneActiva: false, fechaCaducidad: null, mensaje: "" });
...
`;
  ...
  // NUEVA FUNCIÓN: Verificar Asilo
  const handleVerifyAsilo = async () => {
    console.log("🛡️ Verificando Asilo...");
    try {
      if (asiloDocs.length === 0) {
        const mensaje = "ما عندكش أي وثيقة فيها معلومات على اللجوء. هاد شي مزيان للتسوية.";
        setAsiloStatus({ tieneSolicitud: false, tieneDenegacion: false, mensaje });
      ...
      setAsiloStatus({ tieneSolicitud: tieneSolicitudActiva, tieneDenegacion, mensaje });
      await speakFromAutomation(mensaje);
  ...
  // NUEVA FUNCIÓN: Verificar Expulsión
  const handleVerifyExpulsion = async () => {
    console.log("🚫 Verificando Expulsión Europea...");
    try {
      if (expulsionDocs.length === 0) {
        const mensaje = "ماعندكش أي وثيقة فيها قرار الطرد أو الترحيل. هاد شي مزيان بزاف لملف ديالك.";
        setExpulsionStatus({ tieneActiva: false, fechaCaducidad: null, mensaje });
        await speakFromAutomation(mensaje);
        return;
      }
      ...
      setExpulsionStatus({ tieneActiva: tieneExpulsionActiva, fechaCaducidad, mensaje });
      await speakFromAutomation(mensaje);
      
      // Actualizar el estado de expulsión verificado
  ...
  const handleVerifyAll = async () => {
    try {
      ...
      // Mostrar condiciones para depuración
...
`;
      ...
      if (realtimeDcRef.current && realtimeDcRef.current.readyState === "open") {
        ...
        // PRIMERO: Asegurar que Soufiane habla
        await speakFromAutomation(fullSpeech);
        
        // ESPERAR: Dar tiempo a que termine de hablar (aprox 15-20 segundos)
        await new Promise(resolve => setTimeout(resolve, 18000));
        
        // DESPUÉS: Reactivar el micrófono
        await startListening();
  ...
  // Función para forzar desbloqueo manual (solo para pruebas)
  const forceUnlockSoufiane = () => {
    console.log("🔓 FORZANDO DESBLOQUEO MANUAL DE SOUFIANE");
    setSoufianeReady(true);
    toast({ title: "Desbloqueado", description: "Soufiane ha sido desbloqueado manualmente para pruebas" });
    speakFromAutomation("تم فتح الصوت دابا. كيفاش نقدر نعاونك؟");
  };
          ...
          <div className="mt-0 w-full max-w-none lg:col-start-2">
            ...
            {paymentCompleted && (
              <div className="mt-5 space-y-5">
                ...
                {/* NUEVO BOTÓN: Verificar Asilo */}
                <button 
                  onClick={handleVerifyAsilo}
                ...
                {/* NUEVO BOTÓN: Verificar Expulsión */}
                <button 
                  onClick={handleVerifyExpulsion}
                  className="w-[92%] mx-auto h-[52px] rounded-[20px] border border-[#c6922f] bg-[#050816] hover:bg-[#0b1220] transition-all text-white font-medium text-[16px] flex items-center justify-center gap-3 shadow-lg"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-[#d4a94d]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                {/* BOTÓN DE PRUEBA - Forzar desbloqueo (solo para desarrollo) */}
                <button 
                  onClick={forceUnlockSoufiane}
                  className="w-[92%] mx-auto h-[40px] rounded-[20px] bg-yellow-600 hover:bg-yellow-700 text-white text-[12px] font-medium transition-all"
                >
                  🧪 FORZAR DESBLOQUEO DE SOUFIANE (solo pruebas)
                </button>
                ...
                <div className="w-[92%] mx-auto h-[52px] rounded-[20px] border border-[#c6922f]/40 bg-[#050816] flex items-center overflow-hidden shadow-lg">
                  ...
                  <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder={safeLang === "darija" ? "رقم الواتساب" : safeLang === "en" ? "WhatsApp number" : "Número WhatsApp"} className="flex-1 h-full bg-transparent px-4 text-white placeholder:text-white/40 outline-none text-[16px]" />
                </div>
...
function FieldLabel({ label }: { label: string }) {
  return <label className="block text-[12px] font-semibold text-slate-600 mb-1">{label}</label>;
}

function FieldInput({ value, onChange, placeholder }: { value: string; onChange: (value: string) => void; placeholder: string }) {
  return <input value={value} onChange={(e) => onChange(e.target.value)} className="w-[94%] mx-auto rounded-[18px] border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 outline-none focus:border-blue-400" placeholder={placeholder} />;
}

function FieldSelect({ value, onChange, options }: { value: string; onChange: (value: string) => void; options: { value: string; label: string }[] }) {
  return (
    <select value={value} onChange={(e) => onChange(e.target.value)} className="w-[94%] mx-auto rounded-[18px] border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 outline-none focus:border-blue-400">
